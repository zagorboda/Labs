...
double derivative2(double c, double t){
    return ((cos(log(c)) + sin(log(c)) + t) / c);
}
...
int main(){
    ...
    while(1){
        printf("Choose method(1 - bisection, 2 - tangent):");
        method = int_check();
        if(method == 1 || method == 2)
            break;
        else
            printf("Method number is incorrect. Try another one");
    }
    ...
    a2 = float_check();
        ...
        /*switch(f){
            case 1:
                if(func1(a1,t) * func1(a2,t) > 0)
                    printf("Func 1 must have different values on the edges");
                break;
            case 2:
                if(func1(a2,t) * func2(a2,t) > 0)
                    printf("Func 2 must have different values on the edges");
                break;
        }*/
